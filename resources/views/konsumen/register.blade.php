...
<body class="bg-gradient-to-br from-green-100 to-blue-200 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-white p-8 rounded-xl shadow-lg">
        <h2 class="text-3xl font-bold text-center text-blue-700 mb-6">Register Konsumen</h2>
        @if ($errors->any())
        <div class="mb-4 text-red-600 bg-red-100 border border-red-300 rounded px-4 py-2">
            {{ $errors->first() }}
        </div>
        @endif
        <form method="POST" action="{{ route('konsumen.register') }}">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 mb-1">Nama Lengkap</label>
                <input type="text" name="nama_konsumen" class="w-full border border-gray-300 rounded px-3 py-2" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="no_telp" class="w-full border border-gray-300 rounded px-3 py-2" required>
            </div>
            <div class="mb-4">
        ...
        <div class="text-center mt-4">
            <a href="{{ route('konsumen.login') }}" class="text-blue-600 hover:underline">Sudah punya akun? Login</a>
        </div>
    </div>
</body>
